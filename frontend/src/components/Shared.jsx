...
export function SummaryDonut({ title, summary, assets, nameKey = 'ticker', valueKey = 'valor', colors }) {
  ...
  return (
    <div>
      <div className="donut-wrap" style={{ position: 'relative', height: 300 }}>
        ...
        <div style={{
          ...
          transform: 'translate(-50%, -50%)', textAlign: 'center', pointerEvents: 'none', width: 160
        }}>
          <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 2 }}>{title}</div>
          <div style={{ fontSize: 15, fontWeight: 700 }}>{fmt.eur(total)}</div>
...
export function VarCell({ val, pct }) {
  if (val == null) return <td className="hide-mobile" style={{ color: 'var(--text2)' }}>—</td>
